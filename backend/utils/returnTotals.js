...
/**
 * Server-side return totals — GST hardening for Returns/Purchase Returns.
 * Never trust client GST amounts on returns. Server-side recomputation ensures
 * that GST reversals are accurate and audit-compliant.
 *
 * Mirrors recalcSalesTotals/recalcPurchaseTotals pattern.
 */
const PCS_UNITS = ['PCS', 'PC', 'NOS', 'NO'];

/** Qty that drives Amount = Rate × Qty, matching return line qty. */
 ...
 * it already carries fold adjustments that can't be re-derived from qty × rate alone —
 * and only falls back to qty × rate for legacy/incomplete payloads.
 */
...
/**
 * Recompute return totals with server-side GST calculation.
 * Used by both Sales Returns and Purchase Returns.
 *
 * @param {Array} items - Return line items with qty, rate, discounts
 * @param {Object} config - Optional configuration
 * @param {string} config.gstType - 'CGST+SGST' or 'IGST' (defaults to 'CGST+SGST')
 * @param {number} config.gstRate - GST rate as percentage (defaults to 5)
 * @param {Object} config.extras - Additional amounts (lessAmt, discountAmt, rdAmt, addAmt, freight, etc.)
 * @param {string} config.companyGstin - Company GSTIN for state determination
 * @param {string} config.companyStateCode - Company state code
 * @param {string} config.partyGstin - Party GSTIN for state determination
 * @param {string} config.partyStateCode - Party state code
 * @returns {Object} Computed totals with server-side GST amounts
 */
function recalcReturnTotals(items = [], {
  gstType = 'CGST+SGST',
  gstRate = 5,
  extras = {},
  companyGstin,
  ...
  const netAmount = Number((tax.taxableAmount + tax.gstAmount + tax.cess + roundOff).toFixed(2));
...
module.exports = { lineAmount, lineTaxable, recalcReturnTotals };
