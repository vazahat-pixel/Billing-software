...
const OrderModal = ({ isOpen, onClose, initialType = 'Sales' }) => {
  const { parties, items, addOrder, fetchParties, fetchItems } = useStore();
  ...
  // Handle adding item to row grid
  const handleAddItem = () => {
    ...
    // Standard Textiles calculation: Amount = Meters * Rate, fallback to Pieces * Rate
      ...
      <div className="bg-[#FDFCF9] w-full max-w-5xl h-[85vh] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border border-white/20 animate-fadeIn">
        ...
        <div className="px-10 py-6 flex justify-between items-center bg-white border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-2xl font-black text-black tracking-tight italic">{type} Order Registry<span className="text-slate-200">.</span></h2>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Order Management System</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar bg-white">
          ...
          <div className="grid grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Order No (Optional)</label>
              <ERPInput 
                value={orderNo}
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Order Booking Date</label>
              <ERPInput 
                type="date"
                value={date}
        ...
        <div className="px-10 py-6 bg-white border-t border-slate-100 flex justify-between items-center shrink-0">
          <div className="text-right">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block">Aggregate Amount</span>
            <span className="text-2xl font-black text-black">₹ {totalOrderAmount.toFixed(2)}</span>
          </div>
          <div className="flex gap-4">
            ...
            >
              <Save size={16} /> {isSubmitting ? 'Saving...' : 'Book Order'}
            </button>
          </div>
        </div>
