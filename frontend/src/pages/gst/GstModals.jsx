...
export const Gstr1Modal = ({ isOpen, onClose }) => {
  ...
  const invoiceData = useMemo(() => {
    ...
    sales.forEach(s => {
      ...
      if (isRegistered) b2b.push(row);
      else b2cSmall.push(row);
    });

    return { b2b, b2cLarge, b2cSmall, totalTaxable, totalGst };
  }, [sales]);
      ...
      <div className="flex flex-col h-full p-10 space-y-8">
         ...
         <div className="flex gap-4 p-1 bg-slate-100 rounded-xl self-start">
            {[
              ...
              { id: 'b2cSmall', label: 'B2C Small' },
              { id: 'hsn', label: 'HSN Summary' }
            ].map(tab => (
               <button
                 key={tab.id}
                 onClick={() => setActiveTab(tab.id)}
                 className={`px-8 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all ${
                   activeTab === tab.id ? 'bg-white text-black shadow-sm' : 'text-slate-400 hover:text-black'
                 }`}
               >
                 {tab.label}
               </button>
                     ...
                     {invoiceData[activeTab]?.map((inv, index) => (
                        <tr key={index} className="hover:bg-slate-50/50 transition-all group">
                           <td className="px-8 py-5">
                              <p className="text-[11px] font-black text-black uppercase tracking-widest">{inv.invoiceNo}</p>
                              <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">{inv.date}</p>
                           </td>
                           <td className="px-8 py-5 text-[10px] font-bold text-slate-500 uppercase">{inv.partyName}</td>
                           <td className="px-8 py-5 text-[10px] font-bold text-black tracking-widest">{inv.gstin}</td>
                           <td className="px-8 py-5 text-right font-bold text-slate-400 text-[11px]">₹ {inv.taxable.toLocaleString()}</td>
                           <td className="px-8 py-5 text-right font-black text-black text-[12px]">₹ {inv.total.toLocaleString()}</td>
                        </tr>
                     ))}
                  </tbody>
               </table>
