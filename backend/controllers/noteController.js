 ...
 * books DISCOUNT ACCOUNT; the auto-raised note would otherwise double-count both.
 *
...
async function assertNotAutoGenerated(companyId, note, session) {
  ...
  if (note.sourceVoucherId) {
    ...
    let src = null;
    try {
      src = await PaymentVoucher.findOne({ _id: note.sourceVoucherId, companyId }).session(session);
    } catch {
      src = null;
    }
    // Memory-mongo / aborted sessions can make .session(session) miss; fall back.
    if (!src) {
      src = await PaymentVoucher.findOne({ _id: note.sourceVoucherId, companyId });
    }
...
async function unpostNote(companyId, note, req, session) {
  // Auto-generated notes share the source Receipt/Payment journal and must never
  // reverse it or re-apply bill adjustments here. Callers must refuse them first.
  if (note.autoGenerated) {
    throw new Error('Cannot unpost an automatically generated settlement-discount note');
  }
